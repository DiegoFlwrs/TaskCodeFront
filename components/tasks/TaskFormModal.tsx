...
const schema = z.object({
  ...
  status: z.enum(['pendiente', 'en-progreso', 'completada', 'cancelada', 'consultar']),
...
export function TaskFormModal({ open, onClose, onSave, task, date }: TaskFormModalProps) {
  ...
  const appOptions = apps.map((a) => ({ value: a.nombre, label: a.nombre, description: a.descripcion }));
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-card rounded-xl border shadow-xl p-6 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Combobox
                      ...
                      placeholder={ticketOptions.length ? 'Seleccionar ticket...' : 'Sin tickets registrados'}
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Combobox
                      ...
                      value={field.value}
                      onChange={field.onChange}
                      placeholder={appOptions.length ? 'Seleccionar app...' : 'Sin apps registradas'}
            ...
            {/* Horario */}
            <div className={`grid gap-4 ${isEditing ? 'grid-cols-3' : 'grid-cols-1'}`}>
              <div className="space-y-1.5">
