...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      'fixed right-4 top-4 z-[100] flex max-h-screen w-[360px] flex-col space-y-2 sm:w-[400px]',
      className
    )}
    {...props}
  />
...
// Componente de Toast personalizado con iconos
interface CustomToastProps extends Omit<ToastProps, 'type' | 'variant'> {
...
const CustomToast = ({ toastType = 'info', title, description, onClose, ...props }: CustomToastProps) => {
  const icons = {
    success: CheckCircle,
    error: AlertCircle,
  ...
  return (
    <Toast variant={variants[toastType]} {...props}>
